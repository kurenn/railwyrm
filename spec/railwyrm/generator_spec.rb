...
RSpec.describe Railwyrm::Generator do
  class FakeShell
    ...
    def initialize(omit_prompt_files: [])
      @commands = []
      @omit_prompt_files = omit_prompt_files
    end
    ...
    def run!(*command, chdir: nil)
      ...
      if command[0] == "bin/rails" && command[1] == "generate" && command[2] == "claude_on_rails:swarm"
        FileUtils.mkdir_p(File.join(chdir, ".claude-on-rails/prompts"))
        File.write(File.join(chdir, "CLAUDE.md"), "# CLAUDE.md\n\nGenerated by claude-on-rails.\n")
        Railwyrm::Generator::AGENT_PROMPT_APPENDICES.each_key do |prompt_file|
          next if @omit_prompt_files.include?(prompt_file)

          File.write(File.join(chdir, ".claude-on-rails/prompts/#{prompt_file}"), "# #{prompt_file}\n")
        end
      end
  ...
  it "updates Gemfile and schedules all expected setup commands" do
    Dir.mktmpdir do |workspace|
      ...
      claude_md = File.read(File.join(configuration.app_path, "CLAUDE.md"))
      expect(claude_md).to include("Generated by claude-on-rails")
      expect(claude_md).to include("## Untitled UI Component Map")
      expect(claude_md).to include("Ui::Button::Component")
      expect(claude_md).to include("## Architecture Rules")
      expect(claude_md).to include("## Test Conventions")

      views_prompt = File.read(File.join(configuration.app_path, ".claude-on-rails/prompts/views.md"))
      expect(views_prompt).to include("# views.md")
      expect(views_prompt).to include("## Untitled UI Component Rules")

      models_prompt = File.read(File.join(configuration.app_path, ".claude-on-rails/prompts/models.md"))
      expect(models_prompt).to include("validate: true")

      controllers_prompt = File.read(File.join(configuration.app_path, ".claude-on-rails/prompts/controllers.md"))
      expect(controllers_prompt).to include("Turbo Stream")

      tests_prompt = File.read(File.join(configuration.app_path, ".claude-on-rails/prompts/tests.md"))
      expect(tests_prompt).to include("build_stubbed")

      prompt_context = File.read(File.join(configuration.app_path, "PROMPT_CONTEXT.md"))
      expect(prompt_context).to include("vc:")
      expect(prompt_context).to include("prop:")
      expect(prompt_context).to include("slot:")
      expect(prompt_context).to include("stimulus:")
      expect(prompt_context).to include("Ui::Button::Component")
  ...
  it "warns and skips when claude-on-rails does not produce an expected agent prompt" do
    Dir.mktmpdir do |workspace|
      configuration = Railwyrm::Configuration.new(name: "missing_prompt_app", workspace: workspace)
      shell = FakeShell.new(omit_prompt_files: ["views.md"])
      ui = Railwyrm::UI::Buffer.new

      described_class.new(configuration, ui: ui, shell: shell).run!

      warn_log = ui.logs.find { |entry| entry[:level] == "warn" && entry[:message].include?("views.md") }
      expect(warn_log).not_to be_nil
      expect(warn_log[:message]).to include("Did claude-on-rails change its output layout?")

      expect(File).not_to exist(File.join(configuration.app_path, ".claude-on-rails/prompts/views.md"))
      models_prompt = File.read(File.join(configuration.app_path, ".claude-on-rails/prompts/models.md"))
      expect(models_prompt).to include("validate: true")
    end
  end
