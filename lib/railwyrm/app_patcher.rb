...
module Railwyrm
  # Everything that edits an already-generated Rails app in place. `Generator`
  # applies these to an app it just created; `FeatureInstaller` applies them to
  ...
  class AppPatcher
    ...
    attr_reader :app_path, :ui, :shell, :dry_run, :devise_user_model, :display_name
    ...
    def app_display_name
      (display_name || File.basename(app_path)).to_s.tr("-", "_").split("_").map(&:capitalize).join(" ")
    end
