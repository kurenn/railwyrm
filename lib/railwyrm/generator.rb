...
module Railwyrm
  class Generator
    ...
    def enable_optional_devise_modules!(module_names)
      ...
      shell.run!("bin/rails", "db:migrate", chdir: configuration.app_path) if created.any?
    end
    ...
    def next_migration_version
      latest = Dir.glob(File.join(configuration.app_path, "db/migrate/*.rb")).map { |path| File.basename(path).to_i }.max.to_i
